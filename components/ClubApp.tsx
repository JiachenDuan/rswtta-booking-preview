...
import { isParentCancellationAllowed, parentCancellationBlockReason, parentCancellationWarning } from "@/lib/cancellationPolicy";
...
export function ClubApp() {
  ...
  async function cancelParentClass(booking: Booking) {
    ...
    try {
      ...
      try {
        ...
        // Preserve the original mutation error when the clock refresh is unavailable.
      }
      setNotice(message);
      return false;
    } finally {
  ...
  async function updateBooking(
    ...
    try {
      if (status === "cancelled") await cancelBookingAsClub(id);
      else await updateStoredBooking(id, { status, assignedCoach, ...schedule });
      await loadAll();
    ...
    <section className="calendar-first">
      ...
      <section className="support-grid">
        ...
        <section className="section-block">
          ...
          <BookingList bookings={filteredClassBookings} parentActions language={language} now={currentTime.getTime()} onChangeRequest={onChangeRequest} onCancel={onCancel} onComplete={onComplete} />
          {selectedParentBooking ? (
    ...
    <section className="calendar-first">
      ...
      {selectedClubBooking ? (
        <ClubBookingActionModal
          ...
          onAddNewStudent={onAddGroupNewStudent}
          saving={saving}
          onComplete={
...
function ClubBookingActionModal({
  ...
  const newGroupDuplicateCandidates = newGroupNameKey
    ? students.filter((student) => {
        ...
        const phone = student.phone.trim();
        return (
          studentName === newGroupNameKey ||
          studentName.includes(newGroupNameKey) ||
          newGroupNameKey.includes(studentName) ||
          (newGroupStudentEmail.trim() && email === newGroupStudentEmail.trim().toLowerCase()) ||
          (newGroupStudentPhone.trim() && phone === newGroupStudentPhone.trim())
        );
      }).slice(0, 5)
    : [];
          ...
          <div className="group-dropin-panel">
            ...
            {dropInMode === "existing" ? (
              ...
              <div className="tryout-fields">
                ...
                {newGroupStudentName.trim() && (!newGroupStudentEmail.includes("@") || newGroupStudentPhone.trim().length < 7) ? (
                  <p className="modal-info">{copy(language, "Missing email or phone: student will finish profile after first login.", "缺少邮箱或电话：学生首次登录后需要完善资料。")}</p>
                ) : null}
                <button
                  className="primary-button"
                ...
                >
                  <UserPlus size={17} />
                  {copy(language, "Create and add student", "创建并添加学生")}
                </button>
              </div>
            )}
          </div>
...
function GroupClassRequestModal({
  ...
  const cancellationBlockReason = existingEnrollment ? parentCancellationBlockReason(existingEnrollment, now) : null;
  const canCancelEnrollment = Boolean(existingEnrollment && !cancellationBlockReason);
  return (
    <div className="modal-backdrop" role="presentation">
