...
## Failed exact activation rehearsal

A combined execution of the current shadow and activation migrations, using a disposable Club credential and a terminal rollback, failed before activation with SQLSTATE `42702`: `column reference "r.id" is ambiguous` in the login-alias seed block. PostgreSQL aborted the transaction automatically.
...
4. A reviewed migration fix followed by exact rollback, disposable multi-session race/idempotency/direct-access denial tests, full Parent and Club regression, DB-first/client-second deployment, and final hash/UI proof.
