...
do $test$
declare v_definition text; v_acl aclitem[];
begin
  ...
  if not has_function_privilege('anon','public.parent_login(text,text,text)','execute')
    or not has_function_privilege('anon','public.parent_cancel_future_bookings(text,text,uuid,uuid,text,timestamptz,text,timestamptz,integer)','execute')
  ...
  select pg_get_functiondef('public.parent_cancel_future_bookings(text,text,uuid,uuid,text,timestamptz,text,timestamptz,integer)'::regprocedure) into v_definition;
  if position('valid_session' in v_definition)=0 or position('household_accounts' in v_definition)=0
    ...
    or position('operation_nonces' in v_definition)=0 then raise exception 'future cancellation security contract regressed'; end if;
  if exists(select 1 from public.project_rows r join public.project_tables t on t.id=r.project_table_id
    ...
    and r.values ?| array['passwordHash','passwordSalt','confirmationCode']) then raise exception 'public credentials remain'; end if;
