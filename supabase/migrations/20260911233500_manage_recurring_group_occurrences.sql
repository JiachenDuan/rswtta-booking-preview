-- Club-only recurring group occurrence management. No row identity is rewritten.
-- The function selects scope by immutable series/original occurrence identity,
-- locks and validates complete group membership, then moves or soft-cancels atomically.
...
begin
  ...
  if v_count <> 1 then raise exception 'Group management project baseline guard failed: expected 1, found %', v_count; end if;
  ...
  if v_count <> 1 then raise exception 'Group management bookings-table guard failed: expected 1, found %', v_count; end if;
  select id into strict v_bookings_table_id from public.project_tables where project_id = v_project_id and slug = 'bookings';
  select count(*) into v_count from public.project_tables where project_id = v_project_id and slug = 'activity_logs';
  if v_count <> 1 then raise exception 'Group management activity-table guard failed: expected 1, found %', v_count; end if;

  select count(*) into v_count from public.project_rows where project_table_id = v_bookings_table_id;
  if v_count <> 1997 then raise exception 'Group management booking-count baseline guard failed: expected 1997, found %', v_count; end if;
  ...
  if v_count <> 141 then raise exception 'Group block baseline guard failed: expected 141, found %', v_count; end if;
  ...
  if v_count <> 162 then raise exception 'Group-linked row baseline guard failed: expected 162, found %', v_count; end if;
  select count(distinct values->>'seriesId') into v_count from public.project_rows where project_table_id = v_bookings_table_id and values->>'program' = 'Group class';
  if v_count <> 8 then raise exception 'Group series baseline guard failed: expected 8, found %', v_count; end if;
  select count(*) into v_count from public.project_rows where project_table_id = v_bookings_table_id
    and values->>'program' = 'Group class' and coalesce(values->>'status', '') not in ('cancelled', 'coach_confirmed')
    and (values->>'startsAt')::timestamptz > clock_timestamp();
  if v_count <> 128 then raise exception 'Future active group baseline guard failed: expected 128, found %', v_count; end if;
  ...
  where project_table_id = v_bookings_table_id and values->>'program' = 'Group class'
    and (coalesce(values->>'seriesId', '') = '' or coalesce(values->>'recurrenceOccurrenceId', '') = ''
      or coalesce(values->>'recurrenceOriginalStartsAt', '') = '' or coalesce(values->>'groupClassId', '') = '');
  if v_count <> 0 then raise exception 'Group identity baseline guard failed: expected 0 incomplete blocks, found %', v_count; end if;
  select count(*) into v_count from (
    select values->>'groupClassId' from public.project_rows
    where project_table_id = v_bookings_table_id and coalesce(values->>'groupClassId', '') <> ''
    group by values->>'groupClassId'
    having count(*) filter (where values->>'program' = 'Group class') <> 1
  ) broken;
  if v_count <> 0 then raise exception 'Group membership baseline guard failed: expected 0 malformed groups, found %', v_count; end if;
  if to_regprocedure('public.rswtta_canonical_coach_id(text)') is null or to_regprocedure('public.rswtta_booking_ends_at(jsonb)') is null then
    raise exception 'Group management dependency guard failed';
  end if;
  if to_regprocedure('public.manage_group_occurrences(uuid,text,text,text,text,text,text,integer,integer,jsonb,text,text,text)') is not null then
    raise exception 'Group management function baseline guard failed: function already exists';
  end if;
end;
$$;
...
begin
  ...
  if p_action = 'update' then
    ...
    if v_new_selected = v_old_selected and p_new_time_label = v_selected.values->>'timeLabel' then raise exception 'The new group schedule must be different'; end if;
    ...
    for v_block in select * from public.project_rows r where r.id = any(v_target_block_ids) order by r.id loop
      for v_other in select * from public.project_rows r where r.id = any(v_target_block_ids) and r.id > v_block.id order by r.id loop
        if public.rswtta_canonical_coach_id(coalesce(nullif(v_block.values->>'assignedCoach', ''), v_block.values->>'requestedCoach')) =
          ...
          v_block_new_start := (v_block.values->>'startsAt')::timestamptz + v_delta;
          v_block_new_end := v_block_new_start + v_duration;
          v_other_new_start := (v_other.values->>'startsAt')::timestamptz + v_delta;
    ...
    update public.project_rows r set values = r.values || jsonb_build_object(
      'startsAt', to_char(((r.values->>'startsAt')::timestamptz + v_delta) at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
      'dateLabel', to_char(((r.values->>'startsAt')::timestamptz + v_delta) at time zone 'America/Los_Angeles', 'Dy, Mon FMDD, YYYY'),
