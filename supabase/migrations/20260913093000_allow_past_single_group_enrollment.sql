...
begin
  ...
  if to_regclass('private_migration_backups.past_group_manifest_20260913_0952') is null then raise exception 'Past enrollment guard: verified private backup is required'; end if;
  if (select ordered_md5 from private_migration_backups.past_group_manifest_20260913_0952 where kind='bookings') is distinct from (select md5(coalesce(string_agg(md5(row_to_json(r)::text),'' order by id),'')) from public.project_rows r where project_table_id=v_bookings) then raise exception 'Past enrollment guard: booking ordered hash changed after backup'; end if;
  if (select ordered_md5 from private_migration_backups.past_group_manifest_20260913_0952 where kind='accounts') is distinct from (select md5(coalesce(string_agg(md5(row_to_json(r)::text),'' order by id),'')) from public.project_rows r where project_table_id=v_accounts) then raise exception 'Past enrollment guard: account ordered hash changed after backup'; end if;
  if (select ordered_md5 from private_migration_backups.past_group_manifest_20260913_0952 where kind='activity') is distinct from (select md5(coalesce(string_agg(md5(row_to_json(r)::text),'' order by id),'')) from public.project_rows r where project_table_id=v_activity) then raise exception 'Past enrollment guard: activity ordered hash changed after backup'; end if;
end $$;
