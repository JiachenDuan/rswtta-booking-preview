...
begin
 ...
 if found then
  ...
  select coalesce(sum(amount_base_units),0)-v_existing.amount_base_units into v_remaining from public.class_package_events where package_id=v_package.id and version<=v_existing.version;
 ...
 insert into public.class_package_keys(project_id,student_account_id,category,unit_basis) values(v_project,p_student_account_id,p_category,p_unit_basis) on conflict(project_id,student_account_id,category) do nothing;
 select * into strict v_package from public.class_package_keys where project_id=v_project and student_account_id=p_student_account_id and category=p_category;
 if v_package.unit_basis<>p_unit_basis then raise exception 'stored category/unit basis mismatch'; end if;
 select coalesce(sum(amount_base_units) filter(where event_type='opening_set'),0),coalesce(sum(amount_base_units),0),coalesce(max(version),0) into v_opening,v_remaining,v_version from public.class_package_events where package_id=v_package.id;
