-- Cache-isolated collision-safe RPC; retire the original browser-executable route.
begin;
select pg_advisory_xact_lock(hashtextextended('rswtta:club-unverified-legacy-preregistration:v1',0));
do $guard$ begin
 if to_regprocedure('public.club_preregister_student_v2(text,text,text,uuid,jsonb,text)') is not null then raise exception 'v2 RPC already exists'; end if;
 if exists(select 1 from rswtta_private.club_preregistration_requests) then raise exception 'Refuse RPC transition with active preregistration rows'; end if;
end $guard$;
create function public.club_preregister_student_v2(p_club_identifier text,p_club_proof text,p_client_key text,p_request_key uuid,p_input jsonb,p_duplicate_snapshot_hash text) returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions,rswtta_private as $$
