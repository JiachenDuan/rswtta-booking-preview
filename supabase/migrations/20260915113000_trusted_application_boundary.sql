...
begin v_actor:=rswtta_private.require_operator(false); return query select v_actor.membership_id,v_actor.role,v_actor.coach_id,v_actor.status,v_actor.accepted_at; end $$;
...
begin
  ...
  if p_operation in ('suspend','delete') and v_target.membership_id=v_actor.membership_id then raise exception 'Cannot suspend or delete own sole active membership'; end if;
...
begin
  ...
  if p_limit is null or p_limit<1 or p_limit>5000 then raise exception 'Invalid calendar limit'; end if;
  return query select r.id,
    (select a.coach_id from public.booking_coach_assignments a where a.booking_row_id=r.id and a.assignment_kind='assigned'),
    (select a.coach_id from public.booking_coach_assignments a where a.booking_row_id=r.id and a.assignment_kind='requested'),
    (r.values->>'startsAt')::timestamptz,coalesce(r.values->>'dateLabel',''),coalesce(r.values->>'timeLabel',''),coalesce(r.values->>'program',''),coalesce(r.values->>'status',''),coalesce(nullif(r.values->>'studentName',''),left(r.id::text,8))
   from public.project_rows r where r.project_table_id='a7a8a308-2305-4ab6-ad20-5ce174558035'::uuid order by (r.values->>'startsAt')::timestamptz desc,r.id limit p_limit;
end $$;

create or replace function public.operator_project_rows(p_table_slug text,p_limit integer default 5000)
returns table(id uuid,project_table_id uuid,values jsonb,created_at timestamptz,updated_at timestamptz)
language plpgsql stable security definer set search_path=public,rswtta_private,pg_temp as $$
declare v_actor public.project_auth_memberships%rowtype; v_table_id uuid;
begin v_actor:=rswtta_private.require_operator(false); if p_limit is null or p_limit<1 or p_limit>10000 then raise exception 'Invalid row limit'; end if;
 select pt.id into strict v_table_id from public.project_tables pt where pt.project_id=v_actor.project_id and pt.slug=p_table_slug;
 return query select r.id,r.project_table_id,r.values,r.created_at,r.updated_at from public.project_rows r where r.project_table_id=v_table_id order by r.id limit p_limit; end $$;

create or replace function public.operator_mutate_project_row(p_operation text,p_table_slug text,p_row_id uuid,p_values jsonb,p_expected_updated_at timestamptz,p_request_id uuid)
returns table(id uuid,project_table_id uuid,values jsonb,created_at timestamptz,updated_at timestamptz)
language plpgsql security definer set search_path=public,rswtta_private,pg_temp as $$
declare v_actor public.project_auth_memberships%rowtype; v_table_id uuid; v_before public.project_rows%rowtype; v_after public.project_rows%rowtype;
begin
  v_actor:=rswtta_private.require_operator(true);
  if p_operation not in ('create','update','delete') then raise exception 'Invalid row operation'; end if;
  select pt.id into strict v_table_id from public.project_tables pt where pt.project_id=v_actor.project_id and pt.slug=p_table_slug;
  if p_operation='create' then insert into public.project_rows(project_table_id,values) values(v_table_id,coalesce(p_values,'{}')) returning * into v_after;
  else
    if p_expected_updated_at is null then raise exception 'expected_updated_at required'; end if;
    select r.* into strict v_before from public.project_rows r where r.id=p_row_id and r.project_table_id=v_table_id for update;
    if v_before.updated_at is distinct from p_expected_updated_at then raise exception 'Stale project row; reload before changing it'; end if;
    if p_operation='update' then update public.project_rows set values=coalesce(p_values,'{}'),updated_at=clock_timestamp() where project_rows.id=v_before.id returning * into v_after;
    else delete from public.project_rows where project_rows.id=v_before.id returning * into v_after; end if;
  end if;
  perform rswtta_private.append_operator_audit(v_actor,'project_row_'||p_operation,p_request_id,p_table_slug,coalesce(v_after.id,v_before.id)::text,
    jsonb_build_object('status',v_before.values->>'status','program',v_before.values->>'program','date_label',v_before.values->>'dateLabel','time_label',v_before.values->>'timeLabel','starts_at',v_before.values->>'startsAt'),
    jsonb_build_object('status',v_after.values->>'status','program',v_after.values->>'program','date_label',v_after.values->>'dateLabel','time_label',v_after.values->>'timeLabel','starts_at',v_after.values->>'startsAt'));
  return query select v_after.id,v_after.project_table_id,v_after.values,v_after.created_at,v_after.updated_at;
end $$;
...
create or replace function public.operator_set_class_package_opening(
 ...
 p_expected_opening_amount_base_units integer,p_expected_version bigint,p_note text,p_reference text,p_request_id uuid
...
begin
 v_actor:=rswtta_private.require_operator(true);
 select to_jsonb(x) into strict v_result from public.set_class_package_opening(p_student_account_id,p_category,p_unit_basis,p_new_opening_amount_base_units,p_expected_opening_amount_base_units,p_expected_version,p_note,p_reference,p_request_id) x;
 perform rswtta_private.append_operator_audit(v_actor,'class_package_opening_set',p_request_id,'class_package',p_student_account_id::text,
...
revoke all on function rswtta_private.require_aal2(),rswtta_private.require_operator(boolean),rswtta_private.redact_semantic(jsonb),rswtta_private.append_operator_audit(public.project_auth_memberships,text,uuid,text,text,jsonb,jsonb),rswtta_private.guard_project_membership(),rswtta_private.reject_auth_audit_mutation(),rswtta_private.booking_notification_auth_users(uuid) from public,anon,authenticated;
revoke all on function public.operator_accept_invitation(uuid),public.app_my_membership(),public.operator_list_memberships(),public.operator_request_invitation(text,text,text,uuid),public.operator_change_membership_status(uuid,text,uuid),public.operator_request_auth_account_action(text,uuid,text,uuid),public.operator_calendar(integer),public.operator_project_rows(text,integer),public.operator_mutate_project_row(text,text,uuid,jsonb,timestamptz,uuid),public.operator_set_class_package_opening(uuid,text,text,integer,integer,bigint,text,text,uuid),public.parent_my_dashboard() from public,anon,authenticated;
...
begin
 ...
 if has_function_privilege('anon','public.operator_calendar(integer)','execute') or not has_function_privilege('authenticated','public.operator_calendar(integer)','execute') then raise exception 'Operator RPC grant mismatch'; end if;
end $verify$;
commit;
