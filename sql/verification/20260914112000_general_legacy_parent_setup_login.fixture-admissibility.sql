...
begin
  ...
  if (select count(*) from public.project_rows r where r.id in(v_legacy,v_duplicate_a,v_duplicate_b,v_club,v_completed,v_own_booking,v_other_booking))<>7 then raise exception 'Fixture setup incomplete'; end if;
  if (select count(*) from rswtta_private.club_preregistration_aliases a where a.account_id in(v_legacy,v_club))<>2 then raise exception 'Alias fixture setup incomplete'; end if;
end $acceptance$;
rollback;
