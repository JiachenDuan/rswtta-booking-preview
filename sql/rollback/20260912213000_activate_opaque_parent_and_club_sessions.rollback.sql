...
revoke all on function public.parent_login(text,text,text) from anon,authenticated;
revoke all on function public.parent_refresh_session(text,text) from anon,authenticated;
revoke all on function public.parent_logout(text) from anon,authenticated;
revoke all on function public.parent_issue_operation_nonce(text,text) from anon,authenticated;
revoke all on function public.parent_get_account(text) from anon,authenticated;
revoke all on function public.parent_list_bookings(text) from anon,authenticated;
revoke all on function public.parent_list_bills(text) from anon,authenticated;
revoke all on function public.parent_update_account_v2(text,text,jsonb) from anon,authenticated;
revoke all on function public.parent_change_password(text,text,text,text) from anon,authenticated;
revoke all on function public.parent_begin_password_reset(text) from anon,authenticated;
revoke all on function public.parent_complete_password_reset(text,text) from anon,authenticated;
revoke all on function public.parent_cancel_future_bookings(text,text,uuid,uuid,text,timestamptz,text,timestamptz,integer) from anon,authenticated;
revoke all on function public.parent_request_booking_v2(text,text,uuid,jsonb) from anon,authenticated;
